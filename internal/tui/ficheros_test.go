...
func TestDescifrarFicheroDeTexto(t *testing.T) {
	...
	origen := filepath.Join(dir, "mensaje.txt")
	os.WriteFile(origen, []byte("el secreto"), 0o644)

	// Un contenedor de texto, como el que sale de «esfinge cifrar».
	m := nuevo(t)
	sig, _ := m.elegir(accCifrar)
	mm := sig.(modelo)
	msg := trabajarTexto(accCifrar, "el secreto", clave).(listoMsg)
	if msg.err != nil {
		t.Fatal(msg.err)
	}
	_ = mm
	...
	os.WriteFile(comoTexto, []byte(msg.texto+"\n"), 0o644)
