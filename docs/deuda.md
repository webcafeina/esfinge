...
| Adivinar en macOS sale caro y no hay banco de pruebas | Media | Tres versiones seguidas —2.5.1, 2.9.0 y 2.9.1— salieron con un cambio de macOS que no se podía probar aquí, y las tres fallaron: dos no hicieron nada visible y la tercera cerró la aplicación. El ciclo «publicar para ver qué pasa» es lento para quien lo sufre, que es el cliente | Abierto. La salida conocida es `wails build -devtools`, que deja el inspector en la aplicación de verdad: convierte cada corazonada en una prueba en vivo en vez de en una versión. Hace falta darle a `compilar.yml` una entrada para pedirlo |
