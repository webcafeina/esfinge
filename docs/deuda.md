...
| **Nada prueba `pagina.ts` ni la extensión cargada de verdad** | **Alta** | La detección de campos está probada en un Chromium de verdad, y desde la 2.19.0 **el panel también** (`navegador/pruebas/panel.spec.ts`: compilado, con una `chrome` de mentira, incluido el fallo de las tramas de la 2.18.0 reproducido). Lo que sigue sin probar es **la otra mitad del envoltorio**: el guion de página —su guardián de tramas, `yaRellenados`, el observador— y el trabajador de fondo. Ahí vivía el otro fallo de la 2.18.0. La salida sigue siendo **Playwright con la extensión cargada** (`launchPersistentContext` con `--load-extension`) y un host de native messaging de mentira | Abierto · la mitad del panel, saldada el 2026-09-14 |
...
| **Rellenar el código en formularios de verdad** | Media | La detección está probada contra casos escritos aquí —campo declarado, casillas sueltas y en cajas, por el nombre, y cinco casos donde no hay que tocar nada—, no contra los formularios de segundo factor de ahí fuera. Y hay una duda que solo se resuelve mirando: **algunas casillas solo reaccionan a pegar o a pulsaciones de tecla**, así que el código podría quedar puesto y el botón de verificar sin activarse ([ADR 0030](adr/0030-rellenar-el-codigo-de-un-solo-uso.md)) | Abierto · depende del humano, empezando por Cloudflare |
...
| La prueba de la clave de recuperación falla a veces | Baja | Vista una vez el 2026-09-10 en tema claro y no reproducida en tres tandas más. Falla por un `400` en la consola, que en el servidor de desarrollo es **cualquier método de Go devolviendo un error**, no un fallo del servidor: la prueba afirma que la consola queda limpia. Las pruebas comparten una bóveda dentro de la misma tanda —así está pensado—, así que el resultado de ésta depende de lo que haya corrido antes. Mientras sea una de cada cuatro tandas es ruido; si sube, lo que hay que quitar es la dependencia del orden, no la aserción | Abierto |
