...
| **Nada prueba `pagina.ts` ni el camino panel → página** | **Alta** | La detección de campos sí está probada en un Chromium de verdad; lo que la envuelve **no lo prueba nadie**, y ahí han salido los dos fallos de la 2.18.0: el oyente del panel registrado también en las tramas de otro origen —que contestaban antes que la buena— y `yaRellenados` bloqueando un relleno pedido a mano. Los dos los vio el cliente a la primera. La salida está descrita desde el plan de la fase 2: **Playwright puede levantar Chromium con la extensión cargada** (`launchPersistentContext` con `--load-extension`) y se le puede escribir el manifiesto de native messaging dentro de ese perfil, con un host de mentira que conteste JSON preparado. Con eso se ejercita el camino entero sin Esfinge | Abierto · es lo primero de la entrega 3 |
