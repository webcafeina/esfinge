...
| **Rellenar en sitios de verdad** | Alta | La detección de campos está probada contra doce páginas escritas por mí, en un Chromium de verdad. Lo que hay ahí fuera lo han escrito otros: **ninguna prueba puede decir si el formulario de un banco se detecta**. Y con ello van dos preguntas que solo contesta el uso: si rellenar solo resulta demasiado —y entonces hace falta un interruptor en Ajustes— y si un clic más en el panel molesta cuando hay varias cuentas ([ADR 0028](adr/0028-rellenar-en-la-pagina.md)) | Abierto · depende del humano |
| El permiso de sitio en Firefox | Media | En MV3 los `host_permissions` **no se conceden al instalar**: hay que darlos en el panel de extensiones. Si hace falta, la extensión lo dice con una frase que explica dónde, en vez de contestar que ahí no se rellena. Pero no se ha visto en un Firefox de verdad | Abierto · depende del humano |
