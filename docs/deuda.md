...
| El selector de cuentas de Google | Media | Cuando Google enseña «Elige una cuenta» y se pulsa una ficha, **no hay campo de correo** que leer. Quién entra solo se sabe si la página de la contraseña declara el usuario escondido (`autocomplete="username"`); si no lo declara, con una sola cuenta guardada se rellena ésa aunque la ficha sea de otra, y con varias no se rellena nada. **No se sabe qué hace Google**: falta mirar esa página con un diagnóstico por la consola ([ADR 0032](adr/0032-guardar-desde-la-pagina.md)) | Abierto · depende del humano |
