...
| **A2** · **hecha el 2026-09-18, sin publicar** (ADR 0039) | Bienvenida, registro, entrada con código, sincronización. **La identidad para compartir no se hizo** —lo decía esta tabla y no era verdad, visto el 2026-09-23 al empezar la B: no hay semilla, ni Ed25519, ni HPKE en ninguna parte; lo único que existe es el hueco en el protocolo, que acepta unas `llaves` al registrarse y las guarda—. Servidor con `REGISTRO=lista` (solo la casa) | **e2e de dos equipos**: dos `cmd/dev` y dos Vite contra `wrangler dev` local. A crea la cuenta y guarda; B entra y la ve; los dos editan a la vez y la contraseña perdedora sale en el historial; A borra y en B desaparece. **En el Mac: dos máquinas reales** |
...
| **B1** · **hecha el 2026-09-23** | La identidad: semilla en el cuerpo, X25519 y Ed25519 derivadas, huella comparable y su regla de fusión. En Go y en TypeScript. **Sin servidor y sin interfaz** | Pruebas cruzadas: las mismas llaves y la misma huella para la misma semilla, y los dos funden a la misma identidad |
...
| **B3** · **hecha el 2026-09-24** | Invitaciones por correo a quien no tiene cuenta, **con el envío esperando dentro de la bóveda de quien lo manda** y saliendo solo cuando esa dirección publica sus llaves. El correo lleva quién invita y un botón; nada de esto cambia la respuesta del servidor | Pruebas del servidor —incluido que pasarse del tope no delata—, fusión cruzada de la nota, e2e de la ventana y `pruebas-reales` con la extensión cargada. **Cinco invitaciones por cuenta y día**, que caben en los cien correos de Resend |
| **B4** · **hecha el 2026-09-24** | Lo que hay que decir: la política —con quién compartes y la invitación que Webcafeína manda en tu nombre—, las condiciones —compartir, sus topes y la prohibición de usarlo para escribir a desconocidos—, `VERSION_DEL_AVISO` a **3**, las dos fichas, la portada y el README | La copia en texto de la política **se genera desde la web** y `make comprobar` falla si no está al día; el resto, leído |
| **C** · **estudiada el 2026-09-24**, sin empezar | Touch ID o Windows Hello y PIN, como ranuras solo locales. **Y lo investigado cambia de qué va**: sin firmar, en los dos sistemas esto es **un cerrojo y no una llave** —el camino fuerte de macOS exige una entitlement que solo lleva una compilación firmada, y en Windows sin empaquetar la credencial de Hello la puede usar cualquier programa de tu sesión—. Todo en [`desbloqueo-del-sistema.md`](desbloqueo-del-sistema.md), con las cuatro cosas que hay que decidir antes | Solo en Mac y Windows reales |
